...
class PDFReportGenerator:
    ...
    def _build_title_page(self, date_range: str) -> list[Any]:
        ...
        elements.append(
            Paragraph(f"<b>Organization:</b> {self.organization_name}", self.styles["Subtitle"])
        )
        elements.append(Paragraph(f"<b>Report Period:</b> {date_range}", self.styles["Subtitle"]))
        elements.append(
            Paragraph(
            ...
            TableStyle(
                [
                    ...
                    (
                        ...
                        (0, 8),
                        (-1, 8),
                        colors.HexColor("#fed7d7") if critical > 0 else colors.HexColor("#f7fafc"),
                    ),
                    (
                        "BACKGROUND",
                        (0, 9),
                        (-1, 9),
                        colors.HexColor("#feebc8") if high > 0 else colors.HexColor("#f7fafc"),
                    ),
                ]
            )
        )
    ...
    def _build_analyses_section(self, analyses: list[dict[str, Any]]) -> list[Any]:
        ...
        table = Table(
            table_data, colWidths=[2.5 * inch, 0.75 * inch, 1 * inch, 0.75 * inch, 1 * inch]
        )
        table.setStyle(
            TableStyle(
                [
        ...
        if len(analyses) > 50:
            elements.append(Spacer(1, 0.25 * inch))
            elements.append(
                Paragraph(f"<i>Showing 50 of {len(analyses)} analyses</i>", self.styles["Normal"])
            )
    ...
    def _build_findings_section(self, findings: list[dict[str, Any]]) -> list[Any]:
        ...
        for severity in severity_order:
            ...
            for finding in sev_findings[:20]:  # Limit per severity
                ...
                elements.append(Paragraph(f"<b>{title}</b>", self.styles["FindingTitle"]))

                if location:
                    elements.append(
                        Paragraph(f"<i>Location: {location}</i>", self.styles["FindingBody"])
                    )

                description = finding.get("description", "")
                if description:
                    elements.append(Paragraph(description[:500], self.styles["FindingBody"]))
    ...
    def _add_header_footer(self, canvas, doc) -> None:
        ...
        canvas.drawString(
            0.75 * inch, letter[1] - 0.5 * inch, f"CodeVerify Report - {self.organization_name}"
        )
        canvas.drawRightString(
            letter[0] - 0.75 * inch, letter[1] - 0.5 * inch, datetime.utcnow().strftime("%Y-%m-%d")
        )
