...
class WebhookMetrics:
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        avg_attempts = (
            self.total_attempts / self.total_deliveries if self.total_deliveries > 0 else 0
        )
...
class WebhookService:
    ...
    def get_webhooks_for_org(self, organization_id: UUID) -> list[WebhookConfig]:
        ...
        return [
            w for w in self._webhooks.values() if w.organization_id == organization_id and w.enabled
        ]

    def get_webhooks_for_event(self, organization_id: UUID, event: str) -> list[WebhookConfig]:
        """Get webhooks that subscribe to a specific event."""
        return [w for w in self.get_webhooks_for_org(organization_id) if event in w.events]
    ...
    async def process_pending_retries(self) -> int:
        ...
        due_retries = [
            d for d in self._pending_retries if d.next_retry_at and d.next_retry_at <= now
        ]
        ...
        for delivery in due_retries:
            ...
            if not webhook or not webhook.enabled:
                logger.warning(
                    f"Webhook {delivery.webhook_id} not found or disabled, skipping retry"
                )
                delivery.status = WebhookStatus.FAILED
                continue
    ...
    async def emit_event(
        self, organization_id: UUID, event: str, payload: dict[str, Any]
        ...
        tasks = [self.deliver_webhook(webhook, event, payload) for webhook in webhooks]
...
async def emit_analysis_started(
    organization_id: UUID, analysis_id: UUID, repository: str, pr_number: int | None
):
...
async def emit_analysis_failed(
    organization_id: UUID, analysis_id: UUID, repository: str, error: str
):
