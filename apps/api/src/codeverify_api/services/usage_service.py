...
class UsageService:
    ...
    def check_limits(self, organization_id: UUID, resource_type: str) -> tuple[bool, str]:
        """Check if organization is within usage limits."""
        billing = self._billing_info.get(
            organization_id, BillingInfo(organization_id=organization_id)
        )
    ...
    def get_billing_info(self, organization_id: UUID) -> BillingInfo:
        ...
        return self._billing_info.get(organization_id, BillingInfo(organization_id=organization_id))

    def update_billing_tier(self, organization_id: UUID, tier: PlanTier) -> BillingInfo:
...
def record_analysis_usage(organization_id: UUID, analysis_id: UUID):
    ...
    usage_service.record_usage(
        organization_id, "analysis", metadata={"analysis_id": str(analysis_id)}
    )


def record_finding_usage(organization_id: UUID, finding_count: int):
    """Record findings were created."""
    usage_service.record_usage(organization_id, "finding", quantity=finding_count)


def record_api_call(organization_id: UUID, endpoint: str):
    """Record an API call."""
    usage_service.record_usage(organization_id, "api_call", metadata={"endpoint": endpoint})
