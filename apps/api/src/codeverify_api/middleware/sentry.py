...
def set_analysis_context(analysis_id: str, repository: str, pr_number: int | None = None) -> None:
...
def capture_analysis_error(
    error: Exception, analysis_id: str, stage: str, extra: dict[str, Any] | None = None
) -> str:
...
class AnalysisStageSpan:
    ...
    def __enter__(self):
        self.span = sentry_sdk.start_span(
            op=f"analysis.{self.stage_name}", description=f"Analysis stage: {self.stage_name}"
        )
