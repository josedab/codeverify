...
class CreateSessionRequest(BaseModel):
    ...
    ai_assistant: str = Field("github_copilot", description="AI assistant identifier")
    project_context: dict[str, Any] | None = Field(None, description="Additional project context")
...
class AddConstraintRequest(BaseModel):
    ...
    natural_language: str | None = Field(None, description="Human-readable description")
    formal_spec: str | None = Field(None, description="Formal specification (Z3/SMT-LIB)")
    example_violation: str | None = Field(None, description="Example of violation")
    example_correct: str | None = Field(None, description="Correct example")
...
async def create_session(request: CreateSessionRequest) -> SessionResponse:
    ...
    """
    session_id = hashlib.sha256(f"{request.file_path}-{time.time()}".encode()).hexdigest()[:16]
...
async def verify_suggestion(request: VerifySuggestionRequest) -> VerificationResult:
    ...
    active_types = {c["constraint_type"] for c in session["active_constraints"].values()}
...
async def stream_code(request: StreamCodeRequest) -> dict[str, Any]:
    ...
    urgent_issues = [i for i in issues if i.get("severity") in ("critical", "high")]
