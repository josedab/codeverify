...
async def get_dashboard_stats(
    ...
    # Passed analyses
    passed_query = select(func.count(Analysis.id)).where(Analysis.conclusion == "passed")
    if repo_filter:
    ...
    # Failed analyses
    failed_query = select(func.count(Analysis.id)).where(Analysis.conclusion == "failed")
    if repo_filter:
    ...
    for severity in ["critical", "high", "medium", "low"]:
        severity_query = select(func.count(Finding.id)).where(Finding.severity == severity)
    ...
    avg_issues_query = select(
        func.avg(func.coalesce(Analysis.summary["total_issues"].astext.cast(func.INTEGER), 0))
    ).where(Analysis.status == "completed")
...
async def get_repository_stats(
    ...
    total_result = await db.execute(
        select(func.count(Analysis.id)).where(Analysis.repository_id == repository_id)
    )
...
async def get_team_stats(
    ...
    for user, membership in members:
        ...
        dismissed_count = await db.execute(
            select(func.count(Finding.id)).where(Finding.dismissed_by == user.id)
        )
...
async def get_trends(
    ...
    for i in range(period_days):
        ...
        # Analyses count
        analyses_count = await db.execute(select(func.count(Analysis.id)).where(and_(*base_filter)))
    ...
    return TrendsResponse(
        ...
        summary={
            ...
            "avg_findings_per_analysis": total_findings / total_analyses
            if total_analyses > 0
            else 0,
        },
    )
    ...
    for user in members:
        ...
        if metric == "analyses":
            ...
            dismissed = await db.execute(
                select(func.count(Finding.id)).where(
                    Finding.dismissed_by == user.id, Finding.created_at >= start_date
                )
            )
