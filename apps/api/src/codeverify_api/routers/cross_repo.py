...
class ContractModel(BaseModel):
    ...
    contract_type: str = Field(
        "function", description="Contract type: function, class, api_endpoint, event, schema"
    )
    signature: dict[str, Any] = Field(default_factory=dict, description="Contract signature/schema")
...
class ImpactAnalysisRequest(BaseModel):
    ...
    old_contracts: dict[str, dict[str, Any]] = Field(..., description="Old contract versions")
    new_contracts: dict[str, dict[str, Any]] = Field(..., description="New contract versions")
...
@router.get(
    "/repositories", summary="List Repositories", description="List all registered repositories"
)
...
@router.get(
    "/repositories/{owner}/{name}", summary="Get Repository", description="Get repository details"
)
...
async def remove_repository(owner: str, name: str) -> dict[str, Any]:
    ...
    _dependencies = [
        d for d in _dependencies if d["source"] != full_name and d["target"] != full_name
    ]
...
@router.post(
    "/dependencies", summary="Add Dependency", description="Add a dependency between repositories"
)
async def add_dependency(request: DependencyModel) -> dict[str, Any]:
    ...
    if request.source not in _repositories:
        raise HTTPException(
            status_code=404, detail=f"Source repository not found: {request.source}"
        )

    if request.target not in _repositories:
        raise HTTPException(
            status_code=404, detail=f"Target repository not found: {request.target}"
        )
...
@router.get("/dependencies", summary="List Dependencies", description="List all dependencies")
...
@router.post(
    "/contracts", summary="Register Contract", description="Register a contract in the graph"
)
...
@router.get("/contracts", summary="List Contracts", description="List all contracts")
...
@router.get("/contracts/{contract_id}", summary="Get Contract", description="Get contract details")
...
@router.post(
    "/analyze/impact", summary="Analyze Impact", description="Analyze impact of contract changes"
)
async def analyze_impact(request: ImpactAnalysisRequest) -> dict[str, Any]:
    ...
    if breaking_changes > 0:
        recommendations.append(f"This change includes {breaking_changes} breaking change(s)")
        if affected_repos:
            recommendations.append(f"Affected repositories: {', '.join(affected_repos)}")
        recommendations.append("Consider creating a new major version or deprecation period")
...
@router.post("/detect", summary="Detect Dependencies", description="Detect dependencies from code")
...
@router.get("/graph", summary="Get Dependency Graph", description="Get the full dependency graph")
...
@router.get(
    "/graph/cycles", summary="Detect Cycles", description="Detect cycles in the dependency graph"
)
...
def _compare_contracts(
    ...
    if contract_type == "function":
        ...
        new_params = {p["name"]: p for p in new.get("signature", {}).get("parameters", [])}
