...
class RegisterNodeRequest(BaseModel):
    ...
    supported_languages: list[str] = Field(
        default_factory=lambda: ["python", "typescript"], description="Supported languages"
    )
...
@router.get("/nodes", summary="List Nodes", description="List all nodes in the network")
async def list_nodes(
    ...
    if has_capacity:
        nodes = [n for n in nodes if n["active_tasks"] < n["max_concurrent_tasks"]]
...
@router.post(
    "/nodes/{node_id}/heartbeat", summary="Node Heartbeat", description="Send heartbeat from node"
)
async def node_heartbeat(node_id: str) -> dict[str, Any]:
    ...
    pending_for_node = [
        t for t in _tasks.values() if t["assigned_node"] == node_id and t["status"] == "assigned"
    ]
...
@router.delete(
    "/nodes/{node_id}", summary="Remove Node", description="Remove a node from the network"
)
...
async def submit_task(request: SubmitTaskRequest) -> TaskResponse:
    ...
    task_id = hashlib.sha256(f"{time.time()}-{request.code[:50]}".encode()).hexdigest()[:16]
...
@router.get("/tasks/{task_id}", summary="Get Task", description="Get task status and result")
...
@router.post("/tasks/{task_id}/fail", summary="Fail Task", description="Mark task as failed")
...
@router.get("/tasks", summary="List Tasks", description="List tasks with optional filters")
...
@router.get("/health", summary="Network Health", description="Check network health")
...
def _try_assign_task(task_id: str) -> bool:
    ...
    available = [
        ...
        for n in _nodes.values()
        if n["status"] == "online"
        and n["active_tasks"] < n["max_concurrent_tasks"]
        and task["language"] in n["supported_languages"]
    ]
