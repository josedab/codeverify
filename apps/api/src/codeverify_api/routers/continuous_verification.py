...
class VerificationMode(BaseModel):
    ...
    mode: str = Field("standard", description="Verification mode: quick, standard, deep, or full")
...
class IncrementalVerificationRequest(BaseModel):
    ...
    changed_ranges: list[dict[str, int]] | None = Field(
        None, description="Ranges that changed (line_start, line_end)"
    )
    mode: VerificationMode = Field(
        default_factory=VerificationMode, description="Verification mode configuration"
    )
...
class QuickCheckRequest(BaseModel):
    ...
    language: str = Field(..., description="Programming language")
    file_path: str | None = Field(None, description="File path for caching")
...
class DeepVerifyRequest(BaseModel):
    ...
    constraints: list[str] | None = Field(None, description="Custom constraints to verify")
...
class QueueTaskRequest(BaseModel):
    ...
    mode: VerificationMode = Field(
        default_factory=VerificationMode, description="Verification mode"
    )
...
class EditEvent(BaseModel):
    ...
    timestamp: float | None = Field(None, description="Edit timestamp")
...
class QuickCheckResponse(BaseModel):
    ...
    needs_deep_check: bool = Field(..., description="Whether deep verification is recommended")
...
class DeepVerifyResponse(BaseModel):
    ...
    counterexamples: list[str] = Field(default_factory=list, description="Counterexamples found")
    constraints_checked: int = Field(..., description="Number of constraints checked")
...
class IncrementalVerificationResponse(BaseModel):
    ...
    cached_results: int = Field(0, description="Number of results retrieved from cache")
    fresh_results: int = Field(0, description="Number of freshly computed results")
...
class QueueTaskResponse(BaseModel):
    ...
    estimated_wait_ms: float = Field(..., description="Estimated wait time in milliseconds")
...
class QueueStatusResponse(BaseModel):
    ...
    tasks: list[dict[str, Any]] = Field(default_factory=list, description="List of queued tasks")


class PredictionResponse(BaseModel):
    """Response with code completion predictions."""

    predictions: list[str] = Field(default_factory=list, description="Predicted completions")
...
async def queue_task(request: QueueTaskRequest) -> QueueTaskResponse:
    ...
    """
    task_id = hashlib.md5(f"{request.file_path}:{time.time()}".encode()).hexdigest()[:16]
    ...
    for task in _task_queue:
        if task["task_id"] == task_id:
            if task["status"] == "processing":
                raise HTTPException(
                    status_code=400, detail="Cannot cancel task that is already processing"
                )
...
@router.post(
    "/edit", summary="Record Edit", description="Record an edit event for prediction learning"
)
...
@router.post("/cache/clear", summary="Clear Cache", description="Clear the verification cache")
