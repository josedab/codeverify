...
class IncrementalAnalysisEngine:
    ...
    def get_verification_queue(self) -> list[SemanticBlock]:
        ...
        5. Files
        """
        dirty_blocks = [block for block in self.blocks.values() if block.needs_verification]
    ...
    def get_blocks_in_range(self, start_line: int, end_line: int) -> list[SemanticBlock]:
        ...
        for block in self.blocks.values():
            if (
                block.span.contains_line(start_line)
                or block.span.contains_line(end_line)
                or start_line <= block.span.start.line <= end_line
            ):
                result.append(block)
        return result
