...
class ComplianceFrameworkType(str, Enum):  # noqa: UP042
...
class AttestationLevel(str, Enum):  # noqa: UP042
...
class ControlCategory(str, Enum):  # noqa: UP042
    ...
    def _get_soc2_controls(self) -> list[FrameworkControl]:
        ...
        return [
            ...
            _ctrl(
                ...
                [_ET.TEST_RESULT, _ET.CONFIGURATION],
                auto=False,
            ),
        ]
...
class EvidenceVault:
    ...
    def store_evidence(self, evidence: EvidenceArtifact) -> str:
        ...
        if not evidence.content_hash:
            evidence.content_hash = self._compute_hash(self._integrity_payload(evidence))
        self._artifacts[evidence.id] = evidence
        self._hash_index[evidence.content_hash] = evidence.id
        logger.info(
            "evidence_stored", evidence_id=evidence.id, evidence_type=evidence.evidence_type.value
        )
        return evidence.id
    ...
    def verify_integrity(self, evidence_id: str) -> bool:
        ...
        if artifact is None:
            return False
        recomputed = self._compute_hash(self._integrity_payload(artifact))
        return hmac.compare_digest(artifact.content_hash, recomputed)

    @staticmethod
    def _integrity_payload(evidence: EvidenceArtifact) -> str:
        """Build the canonical payload covered by the integrity hash."""
        return f"{evidence.id}:{evidence.evidence_type.value}:{evidence.title}"
...
_ATTESTATION_SECRET_ENV = "CODEVERIFY_ATTESTATION_SECRET"


def _resolve_attestation_secret(secret: str | bytes | None) -> bytes | None:
    """Resolve an injected attestation secret or the configured environment value."""
    resolved = os.environ.get(_ATTESTATION_SECRET_ENV) if secret is None else secret
...
class AttestationEngine:
    ...
    def __init__(self, secret: str | bytes | None = None) -> None:
        self._secret = _resolve_attestation_secret(secret)
    ...
    def create_attestation(
        ...
        attestation = ComplianceAttestation(
            ...
            signature=self._generate_signature(sig_data),
            valid=True,
        )
    ...
    def _generate_signature(self, data: str) -> str:
        ...
        return hmac.new(self._require_secret(), data.encode("utf-8"), hashlib.sha256).hexdigest()
    ...
    def _require_secret(self) -> bytes:
        if self._secret is None:
            raise RuntimeError(
                "Attestation signing requires a secret; pass secret=... to "
                "AttestationEngine or set CODEVERIFY_ATTESTATION_SECRET"
            )
        return self._secret
...
class ComplianceReportGenerator:
    ...
    def _generate_executive_summary(
        ...
        parts = [
            f"Compliance assessment for {framework.value}: {passing}/{total} controls passing ({rate:.0f}%)."
        ]
        if failing:
    ...
    def export_pdf_data(self, report: ComplianceReport) -> dict[str, Any]:
        ...
        return {
            ...
            "summary": {
                ...
                "pass_rate": f"{(report.passing_controls / report.total_controls * 100):.1f}%"
                if report.total_controls
                else "N/A",
            },
...
class ComplianceAsCodeEngine:
    ...
    def __init__(self, attestation_secret: str | bytes | None = None) -> None:
        self.mapper = FrameworkMapper()
        self.vault = EvidenceVault()
        self.attestation_engine = AttestationEngine(secret=attestation_secret)
    ...
    def assess_compliance(
        ...
        for control in controls:
            ...
            attestations = [
                a
                for a in self.attestation_engine._attestations.values()
            ...
            if not matched_rules:
                ...
                status = "failing"
                gaps = [
                    f"Rule '{r}' not passing"
                    for r in matched_rules
                    if not rule_status.get(r, False)
                ]
    ...
    def continuous_monitoring(
        self,
        framework: ComplianceFrameworkType,
        new_evidence: list[EvidenceArtifact],
        ...
        for control in self.mapper.get_controls(framework):
            ...
            gaps = (
                []
                if ev and att
                else (["No attestation on file"] if ev else ["No evidence collected"])
            )
        ...
        logger.info(
            ...
            controls_updated=len(assessments),
        )
        return assessments
    ...
    def compare_periods(
        ...
        return {
            "period1": {
                "start": report1.period_start.isoformat(),
                "end": report1.period_end.isoformat(),
            },
            "period2": {
                "start": report2.period_start.isoformat(),
                "end": report2.period_end.isoformat(),
            },
            "delta": {
