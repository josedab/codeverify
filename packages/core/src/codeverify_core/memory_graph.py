...
class ProofArtifact(BaseModel):
    ...
    def to_storage_dict(self) -> dict[str, Any]:
        ...
        data["id"] = str(self.id)
        data["constraints"] = [
            {**c.model_dump(mode="json"), "id": str(c.id)} for c in self.constraints
        ]
        return data
    ...
    def from_storage_dict(cls, data: dict[str, Any]) -> "ProofArtifact":
        ...
        data["constraints"] = [
            SerializedConstraint(**{**c, "id": UUID(c["id"])}) for c in data.get("constraints", [])
        ]
        return cls(**data)
...
class CrossProjectLearner:
    ...
    def __init__(self, graph: VerificationKnowledgeGraph):
        self.graph = graph
        self._organization_patterns: dict[str, set[str]] = defaultdict(
            set
        )  # org_id -> pattern_hashes
        self._pattern_success_rate: dict[
            str, tuple[int, int]
        ] = {}  # pattern_hash -> (successes, failures)
