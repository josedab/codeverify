...
class InMemoryProofStorage(ProofStorage):
    ...
    def _score_relevance(self, proof: ProofArtifact, query: SearchQuery) -> tuple[float, list[str]]:
    ...
    async def update_stats(self, proof_id: str, success: bool) -> bool:
        ...
        alpha = 0.1
        proof.success_rate = alpha * (1.0 if success else 0.0) + (1 - alpha) * proof.success_rate
...
class ProofArtifactRepository:
    ...
    async def get_repository_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "avg_success_rate": (
                sum(p.success_rate for p in proofs) / len(proofs) if proofs else 0
            ),
            "templates_count": len(self._templates),
        }

    def _count_by_field(self, proofs: list[ProofArtifact], field: str) -> dict[str, int]:
