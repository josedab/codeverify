...
class OrgDependencyGraph:
    ...
    def analyze_transitive_risks(self) -> list[TransitiveRisk]:
        ...
        vulnerable_repos = [repo for repo in self._repos.values() if repo.known_vulnerabilities > 0]
        ...
        for vuln_repo in vulnerable_repos:
            ...
            for dependent, depth in dependents.items():
                ...
                if depth == 1:
                    severity = (
                        RiskLevel.HIGH if vuln_repo.known_vulnerabilities > 5 else RiskLevel.MEDIUM
                    )
                elif depth == 2:
            ...
            if len(component) >= 2:
                ...
                if cohesion >= min_cohesion:
                    ...
                    central = max(
                        component,
                        key=lambda r: (
                            len(self._adjacency.get(r, set()) & component)
...
def reset_org_dependency_analyzer() -> None:
    """Reset the global org dependency analyzer (for testing)."""
    global _org_analyzer
    _org_analyzer = None
