                        ...
                        findings.append(
                            {
                                ...
                                "proof": "Z3 found counterexample where value is null",
                                "proven": True,
                            }
                        )
...
class OfflineModeManager:
    ...
    async def check_offline_readiness(self) -> dict[str, Any]:
        ...
        # Determine readiness
        status["ready"] = status["z3_available"] and (
    ...
    async def analyze_code_offline(
        ...
        if all_findings:
            ...
            penalty = sum(
                severity_weights.get(f.get("severity", "low"), 0.05) for f in all_findings
            )
