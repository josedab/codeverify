...
class CopilotSessionPool:
    ...
    def __init__(
        ...
        self._pools: dict[str, list[tuple[str, float]]] = defaultdict(
            list
    ...
    async def acquire(self, language: str) -> str:
        ...
        async with self._lock:
            ...
            now = time.time()
            pool[:] = [
                (sid, created) for sid, created in pool if now - created < self.session_ttl_seconds
            ]
...
"""
    ...
    async def _mock_stream_response(self, _prompt: str) -> AsyncIterator[str]:
        ...
        mock_response = json.dumps(
            {"findings": [], "summary": "Code looks good - no critical issues found."}
        )
