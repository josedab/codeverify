...
class ParsedQuery:
    ...
    subject: str | None = None  # Variable/expression being asked about
    predicate: str | None = None  # What's being checked
    context: str | None = None  # Additional context (function, line, etc.)

    # Generated formal representation
    z3_query: str | None = None
...
class NaturalLanguageQueryParser:
    ...
    def parse(self, query: str) -> ParsedQuery:
        ...
        query_id = hashlib.sha256(f"{time.time()}-{query}".encode()).hexdigest()[:16]
    ...
    def _generate_z3_query(self, parsed: ParsedQuery) -> str | None:
    ...
    def _generate_constraint(self, parsed: ParsedQuery) -> str | None:
...
class AnswerGenerator:
    ...
    def _generate_null_answer(
        ...
        if result == ProofResult.PROVEN:
            ...
            answer.explanation = f"Found an execution path where {subject} becomes null."
            answer.counterexample = details.get(
                "counterexample",
                {
    ...
    def _generate_bounds_answer(
        ...
        if result == ProofResult.PROVEN:
            ...
            answer.explanation = (
                f"Formal verification proves that {subject} is always valid for the array access."
            )

        elif result == ProofResult.DISPROVEN:
            answer.answer = f"Yes, {subject} can be out of bounds."
            answer.explanation = f"Found a scenario where {subject} exceeds array bounds."
    ...
    def _generate_value_answer(
        ...
        if values:
            answer.answer = (
                f"{subject} can have the following values: {', '.join(map(str, values))}"
            )
    ...
    def _generate_comparison_answer(
        ...
        if result == ProofResult.PROVEN:
            ...
            answer.explanation = "Formal proof confirms the comparison holds in all cases."

        elif result == ProofResult.DISPROVEN:
            answer.answer = f"No, {subject} is not always {op} {other}."
            answer.explanation = "Found cases where the comparison fails."
            answer.counterexample = details.get("counterexample", {})
