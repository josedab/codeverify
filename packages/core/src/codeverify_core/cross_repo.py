...
class ContractAnalyzer:
    ...
    def _analyze_class_change(
        ...
        if removed_methods:
            breaking_reasons.append(f"Methods removed: {', '.join(removed_methods)}")
        ...
        if removed_attrs:
            breaking_reasons.append(f"Attributes removed: {', '.join(removed_attrs)}")
    ...
    def _analyze_api_change(
        ...
        if old_sig.get("method") != new_sig.get("method"):
            breaking_reasons.append(
                f"HTTP method changed from {old_sig.get('method')} to {new_sig.get('method')}"
            )
        ...
        if old_response and new_response:
            ...
            if removed_fields:
                breaking_reasons.append(f"Response fields removed: {', '.join(removed_fields)}")
    ...
    def _analyze_event_change(
        ...
        if added_fields:
            breaking_reasons.append(f"Required fields added: {', '.join(added_fields)}")
