...
class BugPattern:
    ...
    # Pattern definition
    description: str
    code_pattern: str | None = None  # Regex pattern
...
class PatternLearner:
    ...
    def __init__(self) -> None:
        ...
        self._pattern_cooccurrence: dict[str, dict[str, int]] = defaultdict(
            lambda: defaultdict(int)
        )

    def learn_from_bug(self, bug: HistoricalBug) -> BugPattern | None:
    ...
    def _extract_code_pattern(self, bug: HistoricalBug) -> str | None:
...
class RegressionPredictor:
    ...
    def predict(
        ...
        if result.predictions:
            ...
            result.recommendations = self._generate_recommendations(result.predictions)
    ...
    def _generate_recommendations(
        ...
        if "security" in by_type:
            recommendations.append("Review for security vulnerabilities and sanitize inputs")

        # General recommendations
        if len(predictions) > 2:
            recommendations.append("Consider breaking this code into smaller, tested functions")
...
class ModelStorage:
    ...
    def save_learner(self, learner: PatternLearner) -> dict[str, Any]:
        """Save learner state."""
        return {
            "bug_patterns": {k: v.to_dict() for k, v in learner.bug_patterns.items()},
            "revert_patterns": {k: v.to_dict() for k, v in learner.revert_patterns.items()},
...
class OrganizationModel:
    ...
    def train_on_bug(self, bug: HistoricalBug) -> BugPattern | None:
    ...
    def train_on_revert(
        ...
        pattern = self.learner.learn_from_revert(original_commit, revert_commit, reason)
