...
def parse_config(content: str | dict[str, Any]) -> CodeVerifyConfig:
    ...
    if "copilot" in data:
        ...
        config.copilot = CopilotConfig(
            ...
            commands=cp.get(
                "commands", ["verify", "explain", "spec", "trust_score", "fix", "history"]
            ),
        )
...
def should_ignore_finding(
    config: CodeVerifyConfig, file_path: str, category: str
) -> tuple[bool, str]:
...
def passes_thresholds(config: CodeVerifyConfig, findings: dict[str, int]) -> tuple[bool, str]:
    ...
    if critical > config.thresholds.critical:
        return (
            False,
            f"Critical findings ({critical}) exceed threshold ({config.thresholds.critical})",
        )
