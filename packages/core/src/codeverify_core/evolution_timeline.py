...
class TimelineBuilder:
    ...
    def build(self, function_name: str, file_path: str, events: list[dict[str, Any]]) -> FunctionTimeline:
        parsed_events: list[EvolutionEvent] = []
        for e in events:
            try: et = EventType(e.get("type", "modified"))
            except ValueError: et = EventType.MODIFIED
            parsed_events.append(EvolutionEvent(
                event_type=et, commit_sha=e.get("commit", ""),
                author=e.get("author", ""), message=e.get("message", ""),
        ...
        mods = sum(1 for e in parsed_events if e.event_type in (EventType.MODIFIED, EventType.REFACTORED))

        current = "passed" if parsed_events and parsed_events[-1].event_type == EventType.PROOF_PASSED else "unknown"

        return FunctionTimeline(
            function_name=function_name, file_path=file_path,
            events=parsed_events, current_proof_status=current,
            total_modifications=mods, total_proof_passes=passes, total_proof_failures=failures,
        )
...
class TimelineRenderer:
    ...
    def render_mermaid(self, timeline: FunctionTimeline) -> str:
        lines = [f"timeline", f"    title {timeline.function_name} Evolution"]
        for event in timeline.events[-10:]:
            icon = {"proof_passed": "✅", "proof_failed": "❌", "drift_detected": "⚠️",
                    "spec_added": "📝", "bug_fixed": "🔧", "created": "🆕"}.get(event.event_type.value, "📝")
...
class CodeEvolutionService:
    ...
    def record_events(self, function_name: str, file_path: str, events: list[dict[str, Any]]) -> FunctionTimeline:
    ...
    def get_repo_evolution(self, repo: str) -> RepositoryEvolution:
        ...
        return RepositoryEvolution(
            repo=repo, timelines=timelines, total_functions=len(timelines),
...
def get_code_evolution_service() -> CodeEvolutionService:
    ...
    if _evolution_instance is None: _evolution_instance = CodeEvolutionService()
