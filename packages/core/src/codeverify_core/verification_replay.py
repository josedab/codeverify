...
class StateComparison:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "parameter_changes": {
                k: {"old": v[0], "new": v[1]} for k, v in self.parameter_changes.items()
            },
...
class SessionRecorder:
    ...
    def start_session(
        ...
        session_id = hashlib.sha256(f"{time.time()}-{user_id or 'anon'}".encode()).hexdigest()[:16]
    ...
    def end_session(self, session_id: str) -> VerificationSession | None:
...
class ReplayEngine:
    ...
    def _update_state(
        ...
        if event.event_type == EventType.CODE_SUBMITTED:
            ...
            state.constraints = [c for c in state.constraints if c.get("constraint_id") != cid]
...
class SessionStorage:
    ...
    def load(self, session_id: str) -> VerificationSession | None:
