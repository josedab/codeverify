...
class TestLanguageExpansionEngine:
    ...
    def test_rust_detect_language(self):
        from codeverify_core.language_support import SupportedLanguage, detect_language
    ...
    def test_rust_error_handling_check(self):
        from codeverify_core.language_support import SupportedLanguage, Z3ConstraintGenerator
...
class TestCloudSaaS:
    ...
    def test_create_tenant(self):
        from codeverify_core.cloud_saas import TenantManager, TenantStatus, TenantTier
    ...
    def test_oauth_flow(self):
        from codeverify_core.cloud_saas import AuthProvider, OAuthManager
...
class TestIncrementalCache:
    ...
    def test_cache_store_and_retrieve(self):
        from codeverify_core.verification_cache import CacheConfig, VerificationCache
...
class TestLLMCostOptimizer:
    ...
    def test_complexity_classification(self):
        from codeverify_core.llm_cost_optimizer import CheckComplexity, ComplexityClassifier

        classifier = ComplexityClassifier()
        assert classifier.classify("x = 1", "null_safety") == CheckComplexity.TRIVIAL
        long_code = "\n".join([f"line_{i} = {i}" for i in range(50)])
        assert classifier.classify(long_code, "semantic") in (
            CheckComplexity.MODERATE,
            CheckComplexity.COMPLEX,
        )
    ...
    def test_budget_tracking(self):
        from codeverify_core.llm_cost_optimizer import CostBudget, CostOptimizer

        budget = CostBudget(monthly_limit_usd=10.0)
        CostOptimizer(budget=budget)
...
class TestProofExplorer:
    ...
    def test_create_trace_verified(self):
        from codeverify_core.proof_explorer import ProofOutcome, ProofTraceSerializer
    ...
    def test_step_explanations(self):
        ...
        trace = serializer.create_trace(
            "func", "f.py", "null_safety", smt, {"satisfiable": False, "proof_time_ms": 1.0}
        )
...
class TestAutoFixPipeline:
    ...
    def test_generate_null_fix(self):
        from codeverify_core.autofix_loop import Finding, FixGenerator
...
class TestOrgDashboard:
    ...
    def test_metrics_aggregator(self):
        ...
        agg.ingest_finding(
            {"repo": "api", "severity": "high", "category": "null_safety", "rule_id": "null_deref"}
        )
    ...
    def test_roi_calculation(self):
        ...
        agg = MetricsAggregator()
        agg.ingest_finding(
            {"repo": "api", "severity": "critical", "category": "security", "rule_id": "sqli"}
        )
        agg.ingest_finding(
            {"repo": "api", "severity": "low", "category": "style", "rule_id": "naming"}
        )
        roi = agg.compute_roi()
...
class TestVerificationCodeGen:
    ...
    def test_inline_assertion_to_code(self):
        from codeverify_core.verification_codegen import AssertionType, InlineAssertion
    ...
    def test_suggestion_ranking(self):
        from codeverify_core.verification_codegen import GenerationContext, SuggestionRanker
...
class TestPluginMarketplace:
    ...
    def test_validate_manifest(self):
        from codeverify_core.plugin_marketplace import PluginManifest, PluginSDK, PluginType

        valid = PluginManifest(
            name="test", version="1.0", description="desc", plugin_type=PluginType.RULE
        )
    ...
    def test_publish_and_search(self):
        from codeverify_core.plugin_marketplace import PluginManifest, PluginRegistry, PluginType
    ...
    def test_install_plugin(self):
        ...
        registry = PluginRegistry()
        manifest = PluginManifest(
            name="test-plugin", version="1.0.0", description="test", plugin_type=PluginType.AGENT
        )
    ...
    def test_add_review(self):
        ...
        registry = PluginRegistry()
        manifest = PluginManifest(
            name="good-plugin", version="1.0.0", description="great", plugin_type=PluginType.RULE
        )
        registry.publish(manifest)
        review = PluginReview(
            plugin_name="good-plugin", reviewer="user1", rating=5, comment="Excellent!"
        )
    ...
    def test_search_by_type(self):
        ...
        registry = PluginRegistry()
        registry.publish(
            PluginManifest(name="p1", version="1.0", description="d1", plugin_type=PluginType.RULE)
        )
        registry.publish(
            PluginManifest(name="p2", version="1.0", description="d2", plugin_type=PluginType.AGENT)
        )
    ...
    def test_deprecate_plugin(self):
        ...
        registry.publish(
            PluginManifest(
                name="old", version="1.0.0", description="old", plugin_type=PluginType.RULE
            )
        )
    ...
    def test_registry_stats(self):
        ...
        registry = PluginRegistry()
        registry.publish(
            PluginManifest(name="a", version="1.0", description="a", plugin_type=PluginType.RULE)
        )
        registry.publish(
            PluginManifest(name="b", version="1.0", description="b", plugin_type=PluginType.AGENT)
        )
