...
class TestWebhookReceiver:
    ...
    def test_verify_github_signature(self, github_webhook_payload):
        ...
        assert receiver.verify_signature(GitProvider.GITHUB, headers, body)
    ...
    def test_invalid_github_signature(self, github_webhook_payload):
        ...
        assert not receiver.verify_signature(GitProvider.GITHUB, headers, body)
    ...
    def test_verify_gitlab_token(self):
        ...
        assert receiver.verify_signature(GitProvider.GITLAB, headers, b"{}")
...
class TestAirGappedVerifier:
    ...
    def test_initialize_missing_files(self, tmp_path):
        ...
        assert not result
        assert not verifier.model_loaded
    ...
    def test_initialize_with_files(self, tmp_path):
        ...
        config = AirGappedConfig(
            model_path=model_path, rules_path=rules_path, cache_path=tmp_path / "cache"
        )
        verifier = AirGappedVerifier(config)
...
class TestUniversalGitSupport:
    ...
    def test_init_with_credentials(self):
        creds = {
            GitProvider.GITHUB: GitCredentials(provider=GitProvider.GITHUB, token="test-token")
        }
...
class TestDataClasses:
    ...
    def test_git_credentials(self):
        creds = GitCredentials(
            provider=GitProvider.GITHUB, token="token123", api_url="https://api.github.com"
        )
    ...
    def test_webhook_payload(self):
        repo = Repository(provider=GitProvider.GITHUB, owner="owner", name="repo", clone_url="url")
...
class TestIntegration:
    ...
    def test_full_webhook_processing_workflow(self, github_webhook_payload):
        # Setup
        creds = {
            GitProvider.GITHUB: GitCredentials(provider=GitProvider.GITHUB),
        }
        support = UniversalGitSupport(credentials=creds)
    ...
    def test_multi_provider_support(self, github_webhook_payload, gitlab_webhook_payload):
        ...
        gh_payload = support.process_webhook(
            gh_headers, json.dumps(github_webhook_payload).encode()
        )

        # Process GitLab webhook
        gl_headers = {"X-Gitlab-Event": "Merge Request Hook"}
        gl_payload = support.process_webhook(
            gl_headers, json.dumps(gitlab_webhook_payload).encode()
        )
