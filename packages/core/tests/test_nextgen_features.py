...
class TestSmartContractVerification:
    ...
    def test_solidity_no_overflow_post_08(self):
        ...
        analyzer = SolidityAnalyzer()
        report = analyzer.analyze(code)
        overflow_findings = [f for f in report.findings if f.category.value == "integer_overflow"]
    ...
    def test_smart_contract_verifier(self):
        ...
        report = verifier.verify(self.SOLIDITY_CODE, "Vulnerable", ContractLanguage.SOLIDITY)
...
class TestBlastRadius:
    ...
    def test_blast_radius_analysis(self):
        ...
        report = analyzer.analyze("auth-service", ["src/auth.py"], ChangeType.BREAKING)
...
class TestVerificationBudget:
    ...
    def test_budget_downgrade(self):
        ...
        allocations = optimizer.allocate(
            [{"file_path": f"f{i}.py", "risk_score": 0.2} for i in range(20)]
        )
...
class TestNLConversation:
    ...
    def test_classify_followup(self):
        ...
        classifier = IntentClassifier()
        assert (
            classifier.classify("What about the other parameter?", has_history=True)
    ...
    def test_conversation_multi_turn(self):
        ...
        session = ConversationSession(code=code)
        session.ask("Can user be null?")
        session.ask("What about the other parameter?")
