...
class TestRuntimeBridge:
    def test_assertion_translation(self):
        ...
        assertions = t.translate("null_safety", "process", "app.py", ["user"], AssertionLanguage.PYTHON)
    ...
    def test_typescript_assertions(self):
        from codeverify_core.runtime_bridge import AssertionLanguage, AssertionTranslator
        t = AssertionTranslator()
        assertions = t.translate("division_by_zero", "calc", "app.ts", ["x"], AssertionLanguage.TYPESCRIPT)
        assert len(assertions) == 1
    ...
    def test_feedback_loop(self):
        from codeverify_core.runtime_bridge import FeedbackEffect, RuntimeVerificationBridgeService
        svc = RuntimeVerificationBridgeService()
...
class TestIntentRefactoring:
    ...
    def test_equivalence_check_passes(self):
        from codeverify_core.intent_refactoring import EquivalenceResult, IntentPreservingRefactoringService
...
class TestContextWindowVerify:
    def test_consistency_check(self):
        from codeverify_core.context_window_verify import ContextSnippet, ContextWindowVerificationService
    ...
    def test_duplicate_detection(self):
        from codeverify_core.context_window_verify import ConsistencyIssueType, ContextSnippet, ContextWindowVerificationService
        svc = ContextWindowVerificationService()
        snippets = [
        ...
        window = svc.verify_context(snippets)
        dups = [i for i in window.issues if i.issue_type == ConsistencyIssueType.DUPLICATE_DEFINITION]
        assert len(dups) >= 1
...
class TestVerificationReplay:
    def test_session_recording(self):
        ...
        session = svc.record_session("repo", "abc123", [
            {"check_type": "null_safety", "function_name": "get_user", "file_path": "api.py", "result": "pass", "code": "if x is not None: pass"},
    ...
    def test_replay_no_regression(self):
        from codeverify_core.verification_replay_regression import ReplayResult, VerificationReplayService
        svc = VerificationReplayService()
        session = svc.record_session("repo", "v1", [
            {"check_type": "null_safety", "function_name": "fn", "file_path": "a.py", "result": "pass", "code": "x"},
        ], {"a.py": "x"})
    ...
    def test_replay_detects_regression(self):
        ...
        svc = VerificationReplayService()
        session = svc.record_session("repo", "v1", [
            {"check_type": "null_safety", "function_name": "fn", "file_path": "a.py", "result": "pass", "code": "if x is not None: x"},
    ...
    def test_trend_tracking(self):
        from codeverify_core.verification_replay_regression import VerificationReplayService
        svc = VerificationReplayService()
        s = svc.record_session("r", "v1", [{"check_type": "null_safety", "function_name": "f", "file_path": "a.py", "result": "pass", "code": "c"}], {"a.py": "c"})
...
class TestNLProofExplanation:
    def test_counterexample_explanation(self):
        from codeverify_core.nl_proof_explanation import DetailLevel, ExplanationContext, NLProofExplanationService
        svc = NLProofExplanationService()
        ctx = ExplanationContext(
            check_type="division_by_zero", function_name="divide",
            file_path="math.py", line=10,
            variable_assignments={"b": 0}, severity="critical",
        )
    ...
    def test_proof_success_explanation(self):
        from codeverify_core.nl_proof_explanation import ExplanationContext, NLProofExplanationService
        svc = NLProofExplanationService()
        ctx = ExplanationContext(
            check_type="null_safety", function_name="safe_fn", file_path="a.py",
        )
        exp = svc.explain(ctx)
    ...
    def test_pr_comment_generation(self):
        ...
        svc = NLProofExplanationService()
        contexts = [
            ExplanationContext(check_type="null_safety", function_name="f1", file_path="a.py", variable_assignments={"x": None}),
    ...
    def test_detail_levels(self):
        from codeverify_core.nl_proof_explanation import DetailLevel, ExplanationContext, NLProofExplanationService
        svc = NLProofExplanationService()
        ctx = ExplanationContext(check_type="null_safety", function_name="fn", file_path="a.py", line=5, variable_assignments={"x": None})
...
class TestReviewAssignments:
    ...
    def test_reviewer_matching(self):
        ...
        svc.register_reviewer(Reviewer(id="r1", name="Alice", expertise=[ExpertiseArea.SECURITY], seniority="senior"))
        svc.register_reviewer(Reviewer(id="r2", name="Bob", expertise=[ExpertiseArea.GENERAL], seniority="mid"))
    ...
    def test_load_balancing(self):
        from codeverify_core.review_assignments import ExpertiseArea, Reviewer, ReviewAssignmentService
        svc = ReviewAssignmentService()
        svc.register_reviewer(Reviewer(id="r1", name="A", expertise=[ExpertiseArea.GENERAL], current_load=4, max_load=5))
        svc.register_reviewer(Reviewer(id="r2", name="B", expertise=[ExpertiseArea.GENERAL], current_load=0, max_load=5))
...
class TestInvariantPropagation:
    def test_register_and_propagate(self):
        from codeverify_core.invariant_propagation import InvariantPropagationService, InvariantScope
        svc = InvariantPropagationService()
        inv = svc.register_invariant(
            name="null_check_all_inputs", description="All inputs must be null-checked",
            z3_assertion="(assert (not (= x null)))", source_repo="api",
            target_repos=["api", "web"], check_type="null_safety",
        )
        report = svc.propagate(inv.id, {
    ...
    def test_org_scope_propagation(self):
        ...
        svc.register_invariant(
            name="encrypt_passwords", description="Passwords must be encrypted",
            z3_assertion="", source_repo="auth", scope=InvariantScope.ORG,
    ...
    def test_governance_summary(self):
        from codeverify_core.invariant_propagation import InvariantPropagationService, InvariantScope
        svc = InvariantPropagationService()
        svc.register_invariant(name="test", description="test", z3_assertion="",
                               source_repo="r", target_repos=["r"], check_type="null_safety")
...
class TestProofDocs:
    def test_doc_generation(self):
        ...
        doc = svc.generate("API Reference", [
...
class TestGamifiedTraining:
    def test_weakness_analysis(self):
        ...
        areas = svc.analyze_weaknesses("dev1", [
            {"category": "null_safety"}, {"category": "null_safety"},
            {"category": "injection"},
        ])
    ...
    def test_curriculum_generation(self):
        ...
        assert any("null" in l.title.lower() for l in lessons)

    def test_challenge_completion(self):
        from codeverify_core.gamified_training import GamifiedTrainingService
        svc = GamifiedTrainingService()
        svc.analyze_weaknesses("dev1", [{"category": "null_safety"}])
        passed, feedback = svc.submit_challenge("dev1", "test", "if user is not None:\n    greet(user)")
    ...
    def test_skill_level_progression(self):
        ...
        svc = GamifiedTrainingService()
        svc.analyze_weaknesses("dev1", [{"category": "null_safety"}, {"category": "division_by_zero"}, {"category": "injection"}])
        for _ in range(4):
