...
class TestTraceGenerator:
    ...
    def test_trace_marks_violations(self, sample_z3_output, sample_source_code):
        ...
        assertion_steps = [
            s for s in trace.steps if s.step_type in (StepType.ASSERTION, StepType.VIOLATION)
        ]
...
class TestConvenienceFunction:
    ...
    def test_create_playground_with_source(self, sample_z3_output, sample_source_code):
        session = create_playground(
            sample_z3_output, source_code=sample_source_code, function_name="process_data"
        )
...
class TestVariableTypes:
    ...
    def test_variable_dataclass(self):
        var = Variable(
            name="test", value=42, var_type=VariableType.INTEGER, source_location="line:10"
        )
