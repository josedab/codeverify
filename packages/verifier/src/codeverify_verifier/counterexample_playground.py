...
class Z3ModelParser:
    ...
    def parse(self, z3_output: str, source_code: str | None = None) -> Counterexample:
    ...
    def _extract_variables(self, z3_output: str) -> dict[str, Variable]:
        ...
        define_pattern = re.compile(r"\(define-fun\s+(\w+)\s*\(\)\s*(\w+)\s+([^)]+)\)")
        ...
        for match in simple_pattern.finditer(z3_output):
            ...
            if name not in variables and name not in ("sat", "unsat", "model"):
                ...
                value = self._parse_value(value_str.strip(), var_type)
                variables[name] = Variable(name=name, value=value, var_type=var_type)
...
class TraceGenerator:
    ...
    def generate_trace_from_code(
        self, counterexample: Counterexample, source_code: str, function_name: str
    ) -> ExecutionTrace:
        ...
        for i, line in enumerate(lines, 1):
            ...
            if f"def {function_name}" in stripped or f"function {function_name}" in stripped:
                ...
                step = self._create_step(
                    StepType.FUNCTION_CALL, f"Enter {function_name}", i, line, state
                )
                steps.append(step)
                continue
            ...
            # Detect return
                ...
                step = self._create_step(
                    StepType.FUNCTION_RETURN, f"Return: {stripped}", i, line, state
                )
                steps.append(step)
                break

        trace_id = hashlib.sha256(f"{function_name}:{counterexample.ce_id}".encode()).hexdigest()[
            :8
        ]
    ...
    def _evaluate_condition_hint(self, condition: str, state: dict[str, Variable]) -> bool | None:
...
<body>
    ...
    def _render_variables(self, variables: dict[str, Variable], modified: dict[str, Any]) -> str:
    ...
    def _render_trace(self, trace: ExecutionTrace, current_step: int) -> str:
        ...
        for step in trace.steps:
            ...
            source_line = (
                f'<div class="source-line">Line {step.source_line}</div>'
                if step.source_line
                else ""
            )
...
class PlaygroundEngine:
    ...
    def create_session(
        self, z3_output: str, source_code: str | None = None, function_name: str | None = None
    ) -> PlaygroundSession:
        ...
        session = PlaygroundSession(session_id=session_id, counterexample=counterexample)
    ...
    def modify_value(self, session_id: str, variable_name: str, new_value: Any) -> bool:
    ...
    def export_html(self, session_id: str) -> str | None:
    ...
    def export_mermaid(self, session_id: str) -> str | None:
    ...
    def generate_share_link(self, session_id: str) -> str | None:
    ...
    def load_session(self, session_id: str) -> PlaygroundSession | None:
...
def create_playground(
    z3_output: str, source_code: str | None = None, function_name: str | None = None
) -> PlaygroundSession:
