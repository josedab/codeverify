...
"""
        ...
        try:
            ...
            refined = GeneratedSpec(
                ...
                source=SpecificationSource.USER_DEFINED
                if user_feedback
                else SpecificationSource.INFERRED,
    ...
    async def _generate_function_contract(
        ...
        cache_key = hashlib.md5(
            f"{function_name}:{func_info.get('code', full_code)[:500]}".encode()
        ).hexdigest()
...
"""
    ...
    def _build_generation_prompt(
        ...
        parts = [
            "Generate formal specifications for the following function:",
            "",
    ...
    def _calculate_coverage(
        ...
        functions_with_specs = sum(
            1 for c in contracts if c.preconditions or c.postconditions or c.invariants
        )
        ...
        overall = (
            (function_coverage + class_coverage) / 2 if class_invariants else function_coverage
        )
        ...
        return {
            ...
            "avg_function_coverage": sum(c.coverage_score for c in contracts) / len(contracts)
            if contracts
            else 0.0,
        }
