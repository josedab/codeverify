    ...
    def _fix_single_line(self, line: str, finding: Finding, language: str) -> str | None:
        ...
        if finding.category == FixCategory.NULL_SAFETY:
            ...
            if match:
                ...
                return line.replace(
                    f"{var}.{method}()", f"{var}.{method}() if {var} is not None else None"
                )
...
class FixVerifier:
    ...
    def _verify_bounds_check(self, fix: GeneratedFix) -> dict[str, Any]:
        ...
        has_bounds_check = (
            "< len(" in fix.fixed_code
            or ">= 0" in fix.fixed_code
            or "< length" in fix.fixed_code.lower()
        )
    ...
    async def auto_fix(
        ...
        return AutoFixResult(
            ...
            fixes_verified=sum(
                1
                for f in all_fixes
                if f.status == FixStatus.VERIFIED or f.status == FixStatus.READY_FOR_PR
            ),
