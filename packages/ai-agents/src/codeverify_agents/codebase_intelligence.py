...
class CodebaseContext:
    ...
    def to_prompt_context(self) -> str:
        ...
        if self.component:
            parts.append(
                f"Component: {self.component.name} ({self.component.criticality.value} criticality)"
            )
        ...
        if self.similar_bugs:
            bugs_str = "; ".join(f"{b.bug_title} ({b.severity})" for b in self.similar_bugs[:3])
...
class CodebaseIntelligenceEngine:
    ...
    def _determine_criticality(
        ...
        if any(
            keyword in path_lower for keyword in ["auth", "security", "crypto", "payment", "secret"]
        ):
    ...
    def _generate_architectural_notes(
        ...
        if component.bug_count > 0:
            notes.append(f"This file has {component.bug_count} historical bugs - review carefully")
