...
class CodeChangeAnalyzer:
    ...
    def _detect_scope_from_diff(self, diff: str, files: list[str]) -> ChangeScope:
        ...
        config_files = [
            f
            for f in files
            if any(c in f.lower() for c in ["config", "setting", ".yml", ".yaml", ".json", ".env"])
        ]
...
class AlignmentChecker:
    ...
    def _check_file_alignment(
        ...
        all_changed_files = changes.files_modified + changes.files_added + changes.files_deleted
...
class IntentTraceabilityEngine:
    ...
    def _generate_recommendations(
        ...
        if has_scope_mismatch:
            recommendations.append("PR scope differs from ticket. Verify this is intentional.")
        ...
        if has_constraint_violation:
            recommendations.append(
                "⚠️ Changes violate constraints specified in the ticket. This may require approval."
            )
