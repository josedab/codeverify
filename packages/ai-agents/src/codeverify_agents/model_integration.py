...
class ModelConfig:
    ...
    model_id: str | None = None  # HuggingFace model ID
...
class FeatureExtractor:
    ...
    STYLE_FEATURES = [
        "naming_snake_case_ratio",
        "naming_camel_case_ratio",
    ...
    AI_PATTERN_FEATURES = [
        ...
        "has_todo_pattern",
        "has_example_usage",
        "verbose_comment_ratio",
        "perfect_formatting_score",
        "generic_variable_ratio",
    ]
    ...
    def _extract_style(self, code: str, _language: str) -> dict[str, float]:
        ...
        camel_case = sum(1 for i in identifiers if "_" not in i and any(c.isupper() for c in i[1:]))
...
class ONNXModelBackend(BaseModelBackend):
    ...
    def predict(self, features: np.ndarray) -> PredictionResult:
        ...
        return PredictionResult(
            ...
            class_probabilities={
                cls: float(probabilities[i]) for i, cls in enumerate(self.config.output_classes)
            },
        ...
        for _i, probs in enumerate(outputs[0]):
            ...
            results.append(
                PredictionResult(
                    ...
                    class_probabilities={
                        cls: float(probs[j]) for j, cls in enumerate(self.config.output_classes)
                    },
                    inference_time_ms=inference_time,
                    model_version=self.config.version,
                )
...
class HuggingFaceModelBackend(BaseModelBackend):
    ...
    def predict_from_code(self, code: str) -> PredictionResult:
        ...
        inputs = self.tokenizer(
            code, return_tensors="pt", truncation=True, max_length=self.config.max_sequence_length
        )
        ...
        return PredictionResult(
            ...
            confidence=float(probs[predicted_idx]),
            class_probabilities={
                cls: float(probs[i]) for i, cls in enumerate(self.config.output_classes)
            },
            inference_time_ms=inference_time,
            model_version=self.config.version,
        )
...
class AICodeDetector:
    ...
    def __init__(self, config: ModelConfig | None = None):
        self.config = config or ModelConfig(
            name="codeverify-ai-detector", version="1.0.0", backend=ModelBackend.MOCK, threshold=0.5
        )
        self.feature_extractor = FeatureExtractor()
        self.backend: BaseModelBackend | None = None
        self._load_backend()
...
_model_registry.register(
    ModelConfig(name="default", version="1.0.0", backend=ModelBackend.MOCK, threshold=0.5)
)
