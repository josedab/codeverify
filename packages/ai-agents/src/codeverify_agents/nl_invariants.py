...
NL_PATTERNS = [
    ...
    (
        r"(\w+)\s+(?:should|must|shall)\s+(?:be|remain)\s+(?:non-negative|>= ?0|at least 0)",
        ValueConstraint.NON_NEGATIVE,
    ),
    (r"(\w+)\s+(?:should|must|shall)\s+(?:be|remain)\s+negative", ValueConstraint.NEGATIVE),
    (
        r"(\w+)\s+(?:should|must|shall)\s+(?:never|not)\s+(?:be|equal|become)\s+(?:zero|0)",
        ValueConstraint.NON_ZERO,
    ),
    # Null/None constraints
    (
        r"(\w+)\s+(?:should|must|shall)\s+(?:never|not)\s+(?:be|become)\s+(?:null|None|nil)",
        ValueConstraint.NOT_NULL,
    ),
    (
        r"(\w+)\s+(?:is|must be|should be)\s+(?:never|not)\s+(?:null|None|nil)",
        ValueConstraint.NOT_NULL,
    ),
    # Collection constraints
    (
        r"(\w+)\s+(?:should|must|shall)\s+(?:never|not)\s+(?:be|become)\s+empty",
        ValueConstraint.NOT_EMPTY,
    ),
    (
        r"(?:length|size|len)\s+of\s+(\w+)\s+(?:should|must|shall)\s+(?:be|remain)\s+(?:positive|> ?0|at least 1)",
        ValueConstraint.NOT_EMPTY,
    ),
    # Range constraints - captured separately with parameters
    (
        r"(\w+)\s+(?:should|must|shall)\s+(?:be|remain)\s+(?:between|in range)\s+(-?\d+(?:\.\d+)?)\s+(?:and|to)\s+(-?\d+(?:\.\d+)?)",
        "range",
    ),
    (
        r"(\w+)\s+(?:should|must|shall)\s+(?:be|remain)\s+(?:less than|<)\s+(-?\d+(?:\.\d+)?)",
        "less_than",
    ),
    (
        r"(\w+)\s+(?:should|must|shall)\s+(?:be|remain)\s+(?:greater than|>)\s+(-?\d+(?:\.\d+)?)",
        "greater_than",
    ),
    (
        r"(\w+)\s+(?:should|must|shall)\s+(?:be|remain)\s+(?:at most|<=)\s+(-?\d+(?:\.\d+)?)",
        "at_most",
    ),
    (
        r"(\w+)\s+(?:should|must|shall)\s+(?:be|remain)\s+(?:at least|>=)\s+(-?\d+(?:\.\d+)?)",
        "at_least",
    ),
]
...
class NaturalLanguageParser:
    ...
    def _extract_constraint(
        ...
        if constraint_type == "range" and len(groups) >= 3:
            ...
            constraint_type in ("less_than", "greater_than", "at_most", "at_least")
            and len(groups) >= 2
        ):
...
class NaturalLanguageInvariantAgent(BaseAgent):
    ...
    async def analyze(self, code: str, context: dict[str, Any]) -> AgentResult:
        ...
        try:
            ...
            invariant_type = InvariantType(context.get("invariant_type", "invariant"))
...
"""
    ...
    def _calculate_confidence(
        ...
        pattern_matches = sum(1 for c in constraints if c.original_text in original_text.lower())
