...
"""
    ...
    async def analyze_realtime(
        ...
        if result.success:
            ...
            filtered_findings = [
                f for f in findings if self.feedback_learner.should_show_finding(f)
            ]
    ...
    def _build_prompt(self, context: ReviewContext) -> str:
        ...
        # The code to review
        parts.append(
            f"Code to review (lines {context.change_region.start_line}-{context.change_region.end_line}):"
        )
        parts.append("```")
            ...
            for i, raw in enumerate(raw_findings):
                finding = InlineFinding(
                    ...
                    region=CodeRegion(
                        ...
                        end_line=context.change_region.start_line
                        + raw.get("line_end", raw.get("line_start", 1))
                        - 1,
