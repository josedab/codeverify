...
class RegressionOracle(BaseAgent):
    ...
    def _extract_change_metrics(self, diff: str, context: dict[str, Any]) -> ChangeMetrics:
        ...
        functions_modified = sum(
            1
            for line in lines
            if any(p in line for p in func_patterns)
    ...
    def _get_historical_signals(self, file_paths: list[str], author: str) -> HistoricalSignal:
    ...
    async def _semantic_risk_analysis(self, code: str, context: dict[str, Any]) -> dict[str, Any]:
        """Use LLM for semantic risk analysis."""
...
```"""
    ...
    def _combine_scores(self, base_score: float, llm_analysis: dict[str, Any]) -> float:
        ...
        high_severity_count = sum(
            1 for f in llm_analysis.get("risk_factors", []) if f.get("severity") == "high"
        )
    ...
    def _find_similar_bugs(self, code: str, metrics: ChangeMetrics) -> list[dict[str, Any]]:
    ...
    def _compute_bug_similarity(self, code: str, metrics: ChangeMetrics, bug: BugRecord) -> float:
        ...
        if abs(bug.change_metrics.complexity_delta) > 0:
            complexity_ratio = min(
                abs(metrics.complexity_delta), abs(bug.change_metrics.complexity_delta)
            ) / max(abs(metrics.complexity_delta), abs(bug.change_metrics.complexity_delta), 1)
    ...
    def record_bug(self, bug: BugRecord) -> None:
        ...
        # Update file stats
        self._file_bug_counts[bug.file_path] = self._file_bug_counts.get(bug.file_path, 0) + 1
    ...
    def record_change(self, file_path: str, author: str) -> None:
        ...
        self._file_change_counts[file_path] = self._file_change_counts.get(file_path, 0) + 1
    ...
    async def batch_predict(self, changes: list[dict[str, Any]]) -> list[RiskPrediction]:
        """
