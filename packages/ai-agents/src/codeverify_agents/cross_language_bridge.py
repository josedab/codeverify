...
class LanguageAdapter(ABC):
    ...
    @abstractmethod
    def parse_function_signature(self, code: str, function_name: str) -> FunctionContract | None:
        """Parse a function signature from code."""
        pass

    @abstractmethod
    def parse_interface(self, code: str, interface_name: str) -> InterfaceContract | None:
    ...
    def parse_interface(self, code: str, interface_name: str) -> InterfaceContract | None:
        ...
        for method_match in re.finditer(method_pattern, code[class_start:]):
            ...
            if not method_name.startswith("_"):
                func_contract = self.parse_function_signature(code[class_start:], method_name)
                if func_contract:
...
class TypeScriptAdapter(LanguageAdapter):
    ...
    def _parse_parameters(self, params_str: str) -> list[tuple[str, TypeContract]]:
...
class CrossLanguageVerificationBridge(BaseAgent):
    ...
    def _function_contract_to_dict(self, contract: FunctionContract) -> dict[str, Any]:
        ...
        return {
            ...
            "return_type": (
                self._type_contract_to_dict(contract.return_type) if contract.return_type else None
            ),
    ...
    def _interface_contract_to_dict(self, contract: InterfaceContract) -> dict[str, Any]:
    ...
    def get_contract(self, contract_id: str) -> Any:
        ...
        return (
            self._function_contracts.get(contract_id)
            or self._interface_contracts.get(contract_id)
            or self._type_contracts.get(contract_id)
        )
