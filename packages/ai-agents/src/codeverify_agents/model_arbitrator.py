    ...
    def specialization_weighted(
        ...
        for vote in votes:
            ...
            if not profile:
                ...
                spec_bonus = (
                    0.3 if any(s.value in category_lower for s in profile.specializations) else 0
                )
...
class CompetingModelArbitrator(BaseAgent):
    ...
    async def analyze(self, code: str, context: dict[str, Any]) -> AgentResult:
        ...
        try:
            ...
            final_findings = [af for af in arbitrated if af.should_report]
    ...
    async def _arbitrate_finding(
        ...
        should_report = verdict == "confirm" and confidence >= self.report_threshold

        # Adjust severity based on arbitration
        adjusted_severity = self._adjust_severity(finding.severity, confidence, votes)
    ...
    async def _get_model_vote(
        ...
        base_confidence = 0.6
        if profile and any(s.value in finding.category.lower() for s in profile.specializations):
            base_confidence = 0.8
    ...
    def _apply_voting(
        ...
        if self.voting_method == VotingMethod.BORDA_COUNT:
            ...
            confidence = sum(v.confidence for v in votes if v.vote == winner) / max(len(votes), 1)
    ...
    def _evaluate_after_debate(
        ...
        for_confidence = [r.confidence for r in debate_rounds if r.stance == "for"]
        against_confidence = [r.confidence for r in debate_rounds if r.stance == "against"]
    ...
    def _format_result(
        ...
        return {
            ...
            "summary": {
                ...
                "rejected": len(
                    [a for a in all_arbitrated if a.arbitration.final_verdict == "reject"]
                ),
                "uncertain": len(
                    [a for a in all_arbitrated if a.arbitration.final_verdict == "uncertain"]
                ),
            },
            "findings": [
                {
