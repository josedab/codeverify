...
class TestFixTemplates:
    ...
    def test_get_null_safety_template(self):
        template = FixTemplates.get_template(FixCategory.NULL_SAFETY, "python", "guard_check")
        assert template is not None
        assert "{var}" in template

    def test_get_bounds_check_template(self):
        template = FixTemplates.get_template(FixCategory.BOUNDS_CHECK, "python", "bounds_guard")
        assert template is not None
        assert "len(" in template

    def test_nonexistent_template(self):
        template = FixTemplates.get_template(FixCategory.STYLE, "python", "nonexistent")
